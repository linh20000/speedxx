/* global variable for haravan */

/* Image - Map width to size */


/* end global */
(() => {
	var mapWidthImgToSize = {
	pico: 16,
	icon: 32,
	thumb:50,
	small: 100,
...
`;
  ...
  var DragCursor = class extends HTMLElement {
    connectedCallback() {
      ...
      <svg fill="none" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 120 120">
        <path d="M0 60C0 26.863 26.863 0 60 0s60 26.863 60 60-26.863 60-60 60S0 93.137 0 60z" fill="var(--text-color)"/>
        <path d="M46 50L36 60l10 10M74 50l10 10-10 10" stroke="var(--section-background)" stroke-width="4"/>
      </svg>
    `;
    }
    _onMouseEnter(event) {
  ...
  // js/helper/image.js
	...
	function convertNumberToHaravanSize(width){
		console.log(width, 'image width')
		for(const property in mapWidthImgToSize){
						if(width <= mapWidthImgToSize[property]){
								return property;
						}
				}
	}
  ...
  function getSizedMediaUrl(media, size) {
    let src = typeof media === "string" ? media : media ? media["src"] : media["url"];
    size = convertNumberToHaravanSize(size)
		if (size === null) {
      return src;
    }
    if (size === "master") {
  ...
  function getMediaSrcset(media, sizeList) {
    ...
    supportedSizes.forEach((supportedSize) => {
      srcset.push(`${getSizedMediaUrl(media, supportedSize)} ${supportedSize}w`);
    });
    return srcset.join(",");
  }
	// Dev
  function getSupportedSizes(media, desiredSizes) {
    ...
    desiredSizes.forEach((width) => {
				for(const property in mapWidthImgToSize){
						if(width <= mapWidthImgToSize[property]){
								supportedSizes.push(mapWidthImgToSize[property]);
								break;
						}
				}
    });
  ...
  var ShopTheLookNav = class extends CustomHTMLElement {
    ...
    transitionToIndex(index, shouldAnimate = true) {
        ...
        currentElement.animate({ transform: ["translateY(0)", "translateY(-100%)"] }, { duration: shouldAnimate ? 1e3 : 0, easing: "cubic-bezier(1, 0, 0, 1)" });
  ...
  var PredictiveSearchDrawer = class extends DrawerContent {
    ...
    async _doLiquidSearch(term) {
      ...
      supportedTypes.forEach((searchType) => {
        promises.push(fetch(`${window.themeVariables.routes.searchUrl}?section_id=predictive-search-compatibility&q=${term}&type=${searchType}&options[unavailable_products]=${window.themeVariables.settings.searchUnavailableProducts}&options[prefix]=last&view=header`, {
          signal: this.abortController.signal
        }));
      });
    ...
    _supportPredictiveApi() {
     // const shopifyFeatureRequests = JSON.parse(document.getElementById("shopify-features").innerHTML);
			//console.log(shopifyFeatureRequests["predictiveSearch"], 'shopify feautured search')
  ...
  var ProductImageZoom = class extends OpenableElement {
    ...
    async _buildItems() {
      ...
      return Promise.resolve(activeImages.map((item) => {
        const matchedMedia = product["images"].find((media) => media == item.getAttribute("data-media-src")), supportedSizes = getSupportedSizes(matchedMedia, [200, 300, 400, 500, 600, 700, 800, 1e3, 1200, 1400, 1600, 1800, 2e3, 2200, 2400, 2600, 2800, 3e3]), desiredWidth = Math.min(supportedSizes[supportedSizes.length - 1], window.innerWidth);
        console.log('Media matched', matchedMedia)
				return {
        ...
        //  src: getSizedMediaUrl(matchedMedia, `${Math.ceil(Math.min(desiredWidth * window.devicePixelRatio * this.maxSpreadZoom, 3e3))}x`),
          ...
          w: desiredWidth,
          h: parseInt(desiredWidth / window.themeVariables.settings.globalAspectRation)
        };
      }));
    }
  };
  ...
  var ProductMedia = class extends CustomHTMLElement {
    ...
    async _onVariantChanged(event) {
      const variant = event.detail.variant;
			console.log(event, 'on variant change event')
			console.log(this.product, 'product')
      const filteredMediaIds = [];
      let shouldReload = false;
      this.product["images"].forEach((media) => {
        var _a;
        let matchMedia2 = variant["featured_image"] && media === variant["featured_image"]["src"];
  ...
  function formatMoney(cents, format = "") {
    ...
    var value = '';
    var patt = /\{\{\s*(\w+)\s*\}\}/;
    var formatString = (format || Haravan.money_format);
    ...
    switch (formatString.match(patt)[1]) {
		...
		console.log(formatString.replace(patt, value))
  ...
  var QuickBuyDrawer = class extends DrawerContent {
    ...
    _onVariantChanged(event) {
			console.log(event, 'on event change')
      const variant = event.detail.variant;
      if (variant) {
      ...
      if (!this.imageElement || !variant || !variant["featured_image"]) {
        return;
      }
      const featuredMedia = variant["featured_image"];
      ...
      */
      ...
      this.imageElement.setAttribute("src", featuredMedia.src);
  ...
  window.customElements.define("quick-buy-drawer", QuickBuyDrawer);
	...
	function haravanGetMediaSrcset(media, sizeList) {
		 let srcset = [], supportedSizes = typeof media === "string" ? sizeList : haravanGetSupportedSizes(media, sizeList);
     supportedSizes.forEach((supportedSize) => {
	...
	function haravanGetSupportedSizes(media, desiredSizes) {
    ...
    _onVariantChanged(event) {
      ...
      if (featuredMedia["alt"]) {
        this.imageElement.setAttribute("alt", featuredMedia["alt"]);
			...
			/* Giảm bớt thông tin do haravan không cung cấp đủ như shopify */
			
      // this.imageElement.setAttribute("width", featuredMedia["preview_image"]["width"]);
      // this.imageElement.setAttribute("height", featuredMedia["preview_image"]["height"]);
			
			console.log(getSizedMediaUrl(featuredMedia), featuredMedia)
      this.imageElement.setAttribute("src", getSizedMediaUrl(featuredMedia, "small"));
      this.imageElement.setAttribute("srcset", getMediaSrcset(featuredMedia, [114, 228,342]));
    }
  };
  ...
  var ProductVariants = class extends CustomHTMLElement {
    ...
    _onMasterSelectorChanged() {
      ...
      options.forEach((value, index) => {
        let input = this.querySelector(`input[name="option${index + 1}"][value="${CSS.escape(value.toLowerCase())}"], select[name="option${index + 1}"]`), triggerChangeEvent = false;
        if (input.tagName === "SELECT") {
          triggerChangeEvent = input.value !== value;
          input.value = value;
        } else if (input.tagName === "INPUT") {
          triggerChangeEvent = !input.checked && input.value === value.toLowerCase();
          input.checked = input.value === value.toLowerCase();
        }
    ...
    _getVariantFromOptions() {
      ...
      return this.product["variants"].find((variant) => {
        return variant["options"].every((value, index) => value.toLowerCase() === options[index]);
      });
    }
    ...
    _getFirstMatchingAvailableVariant() {
      ...
      do {
        ...
        matchedVariant = this.product["variants"].find((variant) => {
          ...
          return variant["available"] && variant["options"].slice(0, variant["options"].length - slicedCount).every((value, index) => value.toLowerCase() === options[index]);
        });
    ...
    _getSelectedOptionValues() {
      const options = [];
      Array.from(this.querySelectorAll('input[name^="option"]:checked, select[name^="option"]')).forEach((option) => options.push(option.value.toLowerCase()));
      return options;
    }
  ...
  var ProductFacet = class extends CustomHTMLElement {
    ...
    async _rerender(event) {
      ...
      const url = new URL(window.location);
			console.log(url, 'url on trigger event')
    ...
    async _rerenderCart(event) {
      ...
      if (event.detail && event.detail["cart"]) {
        cartContent = event.detail["cart"];
				...
				const reponseMinicart = await fetch(`${window.themeVariables.routes.cartUrl}?view=${this.getAttribute("section")}`);
        html = await reponseMinicart.text();
      } else {
        const response = await fetch(`${window.themeVariables.routes.cartUrl}?view=${this.getAttribute("section")}`);
  ...
  var LineItemQuantity = class extends CustomHTMLElement {
    ...
    async _updateFromLink(link) {
      ...
      const response = await fetch(`${window.themeVariables.routes.cartChangeUrl}.js`, {
        body: JSON.stringify({ line, quantity, sections: ["mini-cart"] }),
        credentials: "same-origin",
        method: "POST",
        headers: {
          "Content-Type": "application/json"
          ...
          <div class="container">
            <div class="cart-notification__wrapper">
              <svg focusable="false" width="20" height="20" class="icon icon--cart-notification" viewBox="0 0 20 20">
                ...
                <path d="M6 10L9 13L14 7" fill="none" stroke="var(--success-color)" stroke-width="2"></path>
              </svg>
        ...
        <div class="cart-notification__overflow">
          <div class="container">
            <div class="cart-notification__wrapper">
              <svg focusable="false" width="20" height="20" class="icon icon--cart-notification" viewBox="0 0 20 20">
                <rect width="20" height="20" rx="10" fill="currentColor"></rect>
                <path d="M9.6748 13.2798C9.90332 13.0555 10.1763 12.9434 10.4937 12.9434C10.811 12.9434 11.0819 13.0555 11.3062 13.2798C11.5347 13.5041 11.6489 13.7749 11.6489 14.0923C11.6489 14.4097 11.5347 14.6847 11.3062 14.9175C11.0819 15.146 10.811 15.2603 10.4937 15.2603C10.1763 15.2603 9.90332 15.146 9.6748 14.9175C9.45052 14.6847 9.33838 14.4097 9.33838 14.0923C9.33838 13.7749 9.45052 13.5041 9.6748 13.2798ZM9.56689 12.1816V5.19922H11.4141V12.1816H9.56689Z" fill="var(--error-color)"></path>
              </svg>
  ...
  var ProductStickyForm = class extends HTMLElement {
    ...
    _onVariantChanged(event) {
      ...
      this.imageElement.setAttribute("width", featuredMedia["preview_image"]["width"]);
      this.imageElement.setAttribute("height", featuredMedia["preview_image"]["height"]);
      this.imageElement.setAttribute("src", getSizedMediaUrl(featuredMedia, 165));
