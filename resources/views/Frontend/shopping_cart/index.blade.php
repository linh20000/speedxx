							...
							<form action="" method="post" novalidate="" class="cart">
								...
								<div class="cart__content">
									<table class="line-item-table table table--loose">
										...
										<tbody class="line-item-table__list cart_list">
											@foreach (Cart::content() as $item)
												<tr class="line-item item_cart" id="cart-item-{{$item->rowId}}" data-id="{{$item->rowId}}">
													<td class="line-item__product">
														<div class="line-item__content-wrapper" >
															<a href="{{route('show_details', [$item->id, Str::slug($item->name)])}}" class="line-item__image-wrapper" tabindex="-1" aria-hidden="true">
																<img class="line-item__image" loading="sizes" sizes="(max-width: 740px) 80px, 92px" alt="" src="{{asset($item->options->thumbnail)}}" srcset="{{asset($item->options->thumbnail)}} 400w, {{asset($item->options->thumbnail)}} 2048w">
															</a>
															<div class="line-item__info">
																<div class="product-item-meta">
																	<a href="{{route('show_details', [$item->id, Str::slug($item->name)])}}" class="product-item-meta__title text--small hidden-tablet-and-up"{{$item->name}}</a>
																	<a href="{{route('show_details', [$item->id, Str::slug($item->name)])}}" class="product-item-meta__title hidden-phone">{{$item->name}}</a>
																	<div class="product-item-meta__property-list " style="display:flex;">
																		...
																		<label class="color-swatch__item" for="option-product-page-1041340280-2-1" style="background-color: {{$item->options->color}}"></label>
																	</div>
																...
																<line-item-quantity class="line-item__quantity hidden-tablet-and-up">
																	<div class="quantity-selector quantity-selector--small">
																		...
																		<a href="" class="quantity-selector__button" aria-label="cart.general.increase_quantity" data-no-instant="">
																	...
																	<span style="cursor:pointer;" class="line-item__remove-button  text--subdued text--xsmall hidden-phone" >Xóa</span>
																</line-item-quantity>
															</div>
														</div>
													</td>
													<td class="line-item__quantity line-item__quantity--block text--center hidden-phone">
														...
														<line-item-quantity style="display: block; margin-top: -4px">
															<div class="quantity-selector quantity-selector--small">
																<a href="" class="quantity-selector__button" aria-label="cart.general.decrease_quantity" data-no-instant="">
																	<svg focusable="false" width="8" height="2" class="icon icon--minus   " viewBox="0 0 8 2">
																		<path fill="currentColor" d="M0 0h8v2H0z"></path>
																	</svg>
																</a>
									...
									<safe-sticky offset="24" class="cart__aside-inner" style="top: 23.8606px;">
										<div class="cart__recap">
											<div class="cart__recap-block">
												...
												<span class="heading h6"><span class="hulkapps-cart-original-total" data-key="">{{Cart::subtotal()}} VND</span></span>
												</div>
											</div>
											...
											<a href="{{route('show_payment')}}" is="loader-button" class="cart__checkout-button checkout-button button button--primary button--full" name="checkout">
					...
					<section>
						<div class="container container--medium">
							<div class="empty-state text-container">
								...
								<a href="{{route('collection_all')}}" class="button button--primary">Hãy xem thêm các sản phẩm của shop nhé !</a>
								</div>
							</div>
