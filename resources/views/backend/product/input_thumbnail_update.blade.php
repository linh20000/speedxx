<div class="">
    <div  class="container">
      <div class="row" id="imgArrList">
        @if ($product->thumbnail_list)
        @foreach (array_keys(JSON_decode($product->thumbnail_list)) as $key)
            <div class="col-2"> 
                <input type="hidden" name="thumbnail_list[]" value="{{JSON_decode($product->thumbnail_list)[$key]}}" />
                <img id="img_arr_prev" src="{{asset(JSON_decode($product->thumbnail_list)[$key])}}"  style=" width: 60px; height: 60px; overflow: hidden; display: flex;justify-content: center;align-items: center;margin: 0 auto;margin-bottom: 20px;" alt="" />
            </div>
        @endforeach
        @else
            <span></span>
